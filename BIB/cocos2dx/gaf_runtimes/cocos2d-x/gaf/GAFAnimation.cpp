...
bool GAFAnimation::init(GAFAsset * anAssetData)
{
	...
	_currentSequenceStart = _currentFrameIndex = FIRST_FRAME_INDEX;
...
{
	_currentSequenceStart = FIRST_FRAME_INDEX;
...
void GAFAnimation::step()
{
	...
	{
		GAFAnimationSequence * seq = _asset->getSequenceByEndFrame(_currentFrameIndex);
		if (seq)
		{
...
bool GAFAnimation::setSequence(const char * name, bool looped, bool _resume, AnimSetSequenceHint hint)
{
	if (!_asset)
	{
		return false;
