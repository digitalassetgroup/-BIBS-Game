...
void GafApi::demo()
{
	...
	GAF::GAFAnimatedObject * object = GAF::GAFAnimatedObject::createAndRun(kAnimationPath, true);
	...
	_asset = GAF::GAFAsset::create(kAnimationPath);
	...
	object = GAF::GAFAnimatedObject::createAndRun(kNamedAnimationPath, true);
	// you can set name of any movieclip in flash and, sprite in code, and use in any way
	GAF::GAFSprite * blue = object->subobjectByName("Blue");
	addToScene(object, 0.5, 0.5);
	if (blue)
	{
		GAF::GAFAnimatedObject * pathObject = _asset->createObjectAndRun(true);
		...
		GAF::GAFMarmaladeGFX::spriteContainerAddSprite(blue->getExternalPointer(), pathObject->getExternalPointer());
		GAF::GAFMarmaladeGFX::setSpritePosition(pathObject->getExternalPointer(), 0, 0);
		GAF::GAFMarmaladeGFX::setSpriteLocator(blue->getExternalPointer(), true);
	}
}

void GafApi::addToScene(GAF::GAFAnimatedObject * object, float px, float py, const char * sequenceName)
{
	GAF::GAFMarmaladeGFX::sceneAddSpriteContainer(this, object->getExternalPointer());
	CCSize size = CCDirector::sharedDirector()->getWinSize();
	GAF::GAFMarmaladeGFX::setSpriteContainerPosition(object->getExternalPointer(), size.width * px, size.height * py);
	if (sequenceName)
	{
...
void GafApi::onFinishSequence(GAF::GAFAnimatedObject * object, const std::string& sequenceName)
{
