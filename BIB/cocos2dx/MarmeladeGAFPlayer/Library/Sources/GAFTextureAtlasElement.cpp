...
bool GAFTextureAtlasElement::initWithDictionary(CCDictionary * aDictionary)
{
	...
	{
		bounds = GAFCCRectMake(nX->getValue(), nY->getValue(), nWidth->getValue(), nHeight->getValue());
	}
	else
	{
		GAFLOGERROR("Error while creating GAFTextureAtlasElement from NSDictionary. One or more size attributes missing.");
		return false;
	}
	...
	{
		GAFLOGERROR("Error while creating GAFTextureAtlasElement from NSDictionary. One or more pivot attributes missing.");
		return false;
	}
	...
	{
		atlasIdx = 0;
		GAFLOGWARN("AtlasID is missing, assuming first atlas is used.");
	}
	return true;
}
