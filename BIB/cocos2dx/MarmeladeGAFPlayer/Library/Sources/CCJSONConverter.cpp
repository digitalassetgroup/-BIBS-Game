...
{
    GAFAssert(dictionary, "CCJSONConverter:can not convert a null pointer");
...
CCDictionary * CCJSONConverter::dictionaryFrom(const char *str)
{
    ...
    GAFAssert(json && json->type==cJSON_Object, "CCJSONConverter:wrong json format");
...
{
    GAFDICT_FOREACH(dictionary, pElement)
	{
        CCObject * obj = pElement->second;
        cJSON * jsonItem = getObjJson(obj);
        cJSON_AddItemToObject(json, pElement->first.c_str(), jsonItem);
    }
}
...
cJSON * CCJSONConverter::getObjJson(CCObject * obj)
{
    ...
    GAFLog("CCJSONConverter encountered an unrecognized type");
    return NULL;
}
