...
void GAFAsset::setScalingParameters(float currentDeviceScale, bool makeExactScaleForObject)
{
...
float GAFAsset::getAtlasSclae(CCDictionary * atlas)
{
	if (!atlas)
	{
		return 0;
	}
	CCNumber * scale = (CCNumber * )atlas->objectForKey(kAtlasScaleKey);	
...
bool GAFAsset::initWithImageData(const std::string& jsonPath)
{
	...
	_textureAtlas = GAFTextureAtlas::create(fp.c_str(), atlasDictionary, atlasesInfo);
	if (!_textureAtlas)
	{
...
void GAFAsset::loadFramesFromConfigDictionary(CCDictionary * aConfigDictionary)
{
    ...
    CCDICT_FOREACH(_objects, pElement)
    {
		...
		GAFSubobjectState *state = GAFSubobjectState::createWithObjectId(key);
...
GAFAnimationSequence * GAFAsset::getSequenceByEndFrame(int frame)
{
