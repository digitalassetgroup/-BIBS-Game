...
CCArray * GAFAsset::objectStatesFromConfigFrame(CCDictionary * configFrame)
{
    ...
    GAFDICT_FOREACH(stateNodes, pElement)
    {
		CCDictionary * stateDictionary = (CCDictionary *)pElement->second;
		GAFSubobjectState * state = GAFSubobjectState::createWithStateDictionary(stateDictionary, pElement->first.c_str());
		if (state)
		{
		...
		{
			GAFLOGWARN("GAFSubobjectState cannot be created. Ignoring.");
		}
	}
    return states;
}
...
void GAFAsset::loadAnimationSequences(CCArray * aSequencesNodes)
{
	...
	for (unsigned int i = 0; i < aSequencesNodes->count(); ++i)
	{
		...
		{
			GAFLOGERROR("Error while creating GAFAsset. aSequencesNodes cannot be parsed.");
		}
	}
...
{
	GAFAssert(_objects, "objects != NULL");
    ...
    GAFDICT_FOREACH(_objects, pElement)
    {
		const char * key = pElement->first.c_str();
	...
	for (unsigned int index = 0; index < configFrameCount; ++index)
	{
		if (configFrameIndex < animationConfigFrames->count())
		{
			...
			if (int(configFrameNo - 1) == int(index))
			{
				...
				for (unsigned int j = 0; j < newStates->count(); ++j)
				{
		...
		GAFDICT_FOREACH(currentStates, s)
		{
			allValues->addObject(s->second);
		}
...
GAFAnimationSequence * GAFAsset::getSequenceByLastFrame(int frame)
{
	...
	GAFDICT_FOREACH(_animationSequences, s)
	{
		GAFAnimationSequence * seq = (GAFAnimationSequence*) s->second;
