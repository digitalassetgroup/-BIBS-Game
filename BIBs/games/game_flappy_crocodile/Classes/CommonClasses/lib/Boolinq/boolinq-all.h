#pragma once
#ifndef boolinq_all_h
#define boolinq_all_h
...
namespace boolinq
{
    ...
    template<typename R, typename F>
    typename R::value_type min(R r, F f, typename R::value_type defaultValue)
    {
		if (r.empty())
			return defaultValue;

        typename R::value_type min = r.front();
        while (!r.empty())
        {
            if (f(r.front()) < f(min))
                min = r.front();
    ...
    template<typename R>
    typename R::value_type min(R r, typename R::value_type defaultValue = R::value_type())
    {
        typedef typename R::value_type value_type;
        return min(r,[](const value_type & a)->value_type{return a;}, defaultValue);
    }
    ...
    {
	private:
		typedef typename R::value_type TRValueType;

    public:
        typename R::value_type min(typename R::value_type defaultValue = TRValueType()) const
        {
            return boolinq::min(((TLinq<R>*)this)->r, defaultValue);
        }

        template<typename F>
        typename R::value_type min(F f, typename R::value_type defaultValue = TRValueType()) const
        {
            return boolinq::min(((TLinq<R>*)this)->r,f, defaultValue);
        }
    };
}
// namespace boolinq
...
namespace boolinq
{
    ...
    template<typename R, typename F>
    typename R::value_type max(R r, F f, typename R::value_type defaultValue)
    {
		if (r.empty())
			return defaultValue;

        typename R::value_type max = r.front();
        while (!r.empty())
        {
            if (f(r.front()) > f(max))
                max = r.front();
    ...
    template<typename R>
    typename R::value_type max(R r, typename R::value_type defaultValue = R::value_type())
    {
        typedef typename R::value_type value_type;
        return max(r,[](const value_type & a)->value_type{return a;}, defaultValue);
    }
    ...
    public:
        typename R::value_type max(typename R::value_type defaultValue = TRValueType()) const
        {
            return boolinq::max(((TLinq<R>*)this)->r, defaultValue);
        }

        template<typename F>
        typename R::value_type max(F f, typename R::value_type defaultValue = TRValueType()) const
        {
            return boolinq::max(((TLinq<R>*)this)->r,f, defaultValue);
        }
    };
}
// namespace boolinq
...
namespace boolinq
{
    ...
    public:
        template<typename F>
        void for_each_linq(F f) const
        {
            return boolinq::for_each_linq(((TLinq<R>*)this)->r,f);
        }
    };
}
// namespace boolinq
...
namespace boolinq
{
    ...
    class SelectRange
    {
        template<typename FF, typename TArg>
        static auto get_return_type(FF * f = NULL, TArg * arg = NULL)
                    -> decltype((*f)(*arg));
    public:
...
namespace boolinq
{
    ...
    public:
        ...
        WhereRange_withKey(R r, F f, const value_type & key)
            : WhereRange<R, ReturnEquals_gb<typename R::value_type,F> >(r, ReturnEquals_gb<value_type,F>(key,f))
            , k(f(key))
        {
        }
