...
#import "nsdata_and_base64.h"

#ifdef USE_ANTI_FRAUD_RECEIPT_CHECK
	void oniTCreceipitVerification(const char * productId, bool ok);
#endif
...
- (NSDictionary *)dictionaryFromPlistData:(NSData *)data
{
    ...
    if (!dictionaryParsed)
    {
        if (error)
        {
            NSLog(@"OMG: Error while parsing NSDictionary");
        }
        return nil;
    }
...
- (BOOL)verifyPurchase:(SKPaymentTransaction *)transaction
{
    ...
    NSString *jsonObjectString = [self encodeBase64:(uint8_t *)transaction.transactionReceipt.bytes
                                             length:(NSInteger)transaction.transactionReceipt.length];
...
- (BOOL)doesTransactionInfoMatchReceipt:(NSString*) receiptString
{
	...
	const char * productId = [[verifiedReceiptReceiptDictionary objectForKey:@"product_id"] UTF8String];
    ...
    {
#ifdef USE_ANTI_FRAUD_RECEIPT_CHECK
		oniTCreceipitVerification(productId, false);
#endif
        return NO;
    }
...
#ifdef USE_ANTI_FRAUD_RECEIPT_CHECK
	oniTCreceipitVerification(productId, failCount == 0);
#endif
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data
{
    ...
    if (isOk)
    {
        NSLog(@"validation succedeed %@", responseString);
    }
    else
    {
        NSLog(@"validation falied %@", responseString);
    }
...
BOOL checkReceiptSecurity(NSString *purchase_info_string, NSString *signature_string, CFDateRef purchaseDate)
{
    ...
    uint8_t *purchase_info_bytes = NewBase64Decode([purchase_info_string cStringUsingEncoding:NSASCIIStringEncoding],
                                                   [purchase_info_string length],
                                                 &purchase_info_length);
    
    size_t signature_length;
    uint8_t *signature_bytes = NewBase64Decode([signature_string cStringUsingEncoding:NSASCIIStringEncoding],
...
{
    NSData *data = [NSData dataWithBytes:input length:(NSUInteger)length];      
