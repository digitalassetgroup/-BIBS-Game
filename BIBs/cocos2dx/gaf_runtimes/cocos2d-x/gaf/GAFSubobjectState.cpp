...
GAFSubobjectState * GAFSubobjectState::createWithObjectId(const char * objectId)
{
	GAFSubobjectState * ret = new GAFSubobjectState();
	if (ret && ret->initWinthObjectId(objectId))
	{
		ret->autorelease();
		return ret;
	}
...
bool GAFSubobjectState::initWinthObjectId(const char * objId)
{
	objectId = objId;
	zIndex = 0;
...
bool GAFSubobjectState::initWithStateDictionary(CCDictionary * dict, const char * objId)
{
	...
	if (color)
	{
		...
		_colorOffsets[GAFCTI_A] = atof(color[0].c_str()) / 255.0f;
		
		_colorMults[GAFCTI_R] = atof(color[1].c_str()) ;
		_colorOffsets[GAFCTI_R] = atof(color[2].c_str()) / 255.0f;
		
		_colorMults[GAFCTI_G] = atof(color[3].c_str());
		_colorOffsets[GAFCTI_G] = atof(color[4].c_str()) / 255.0f;
		
		_colorMults[GAFCTI_B] = atof(color[5].c_str());
		_colorOffsets[GAFCTI_B] = atof(color[6].c_str()) / 255.0f;
	}
	else
	{
	...
	if (effects)
	{
		...
		for (int i = 0; i < effects->count(); ++i)
		{
			...
			if (type)
			{
				if (0 == strcmp(type->getCString(), kBlurFilterName))
				{
					...
					if (x && y)
					{
						...
						_filters->setObject(blurFilter, kBlurFilterName);
					}
				}
			}
			else
