...
bool GAFSubobjectState::initEmptyWinthObjectId(const char * objId)
{
	objectId = objId;
	zIndex = 0;
	affineTransform = GAFAffineTransformMake(1,0,0,1,0,0);
...
bool GAFSubobjectState::initWithStateDictionary(CCDictionary * dict, const char * objId)
{
	if (!objId)
	{						
		GAFLOGERROR("Error while creating GAFSubobjectState. Invalid objectId");
		return false;
	}
	objectId = objId;
	...
	if (!state)
	{
		GAFLOGERROR("Error while creating GAFSubobjectState. State state is missing");
		return false;
	}
	std::string str(state->getCString());
	unsigned int s = str.find('{');
	unsigned int e = str.find('}');
	if (s == std::string::npos || e == std::string::npos || e < s)
	{
		GAFLOGERROR("Error while creating GAFSubobjectState. Initial state data '%s'", str.c_str());
		return false;
	}
	...
	if (6 != transform.size())
	{
		GAFLOGERROR("Error while creating GAFSubobjectState. Initial transform data '%s'", temp.c_str());
		return false;
	}
	affineTransform = GAFAffineTransformMake(
	...
	if (color)
	{
		...
		if (7 != color.size())
		{
			GAFLOGERROR("Error while creating GAFSubobjectState. Invalid color data '%s'", c.c_str());
			return false;
		}
	...
	if (effects)
	{
		...
		for (int i = 0; i < effects->count(); ++i)
		{
			...
			{
				GAFLOGWARN("Can not determine effect type, skipping it");
			}			
		}
	}
	return true;	
